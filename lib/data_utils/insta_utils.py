...
def concatenate_annotations():
    ...
    for i in range(273):
        filename = osp.join(GLoT_DB_DIR, 'insta_parts', f'insta_train_part_{i}.h5')
        print(filename)
    ...
    db_file = osp.join(GLoT_DB_DIR, 'insta_train_db.h5')
...
if __name__ == '__main__':
    ...
    os.makedirs(osp.join(GLoT_DB_DIR, 'insta_parts'), exist_ok=True)

    for idx, fp in enumerate(fpaths):
        dataset = read_single_record(fp)

        db_file = osp.join(GLoT_DB_DIR, 'insta_parts', f'insta_train_part_{idx}.h5')
