...
def plot_accel(joints_pred, joints_gt, out_dir, name='', other_preds=['./plot/meva_accel_pred_basketball.npy', './plot/vibe_accel_pred_basketball.npy']):
    ...
    if False and other_preds:
        ...
        plt.plot(time, accel_vibe * 1000, label='GLoT', color='#65D491')
    ...
    plt.plot(time, accel_err*1000, label='GLoT (Ours)', color='#FF7F0E') #
    ...
    plot_name = f'{out_plot_dir}/GLoT_accel_pred_error_{name}.png'
    print("...save plot to ", plot_name)
    plt.savefig(plot_name, bbox_inches = 'tight')
    np.save(f'{out_plot_dir}/GLoT_accel_pred_{name}', accel_pred)
